class Heap {
    constructor(arr, order, prop = null, secProp = null) {
        this._arr = arr;
        this._order = order;
        this._prop = prop ? prop : null;
        this._secProp = secProp ? secProp : null;
    }
    pop() {
    ...
    _heapify(startIndex = 0) {
        ...
        if (child1Exists && !this._compare(this._arr[startIndex], this._arr[child1], false, this._prop, true)) {
            if (child2Exists && this._compare(this._arr[child1], this._arr[child2], false, this._prop, true)) {
        ...
        else if (child2Exists && !this._compare(this._arr[startIndex], this._arr[child2], false, this._prop, true)) {
            [this._arr[startIndex], this._arr[child2]] = [this._arr[child2], this._arr[startIndex]];
            finalIndex = child2;
        }
        else
    ...
    _compare(a, b, equals, property = this._prop, secCompare = false) {
        if (!secCompare){
            if (!property) {
                if (!equals) {
                    if (this._order == "max") {
            ...
            if (a[property] == b[property]){
                return this._reverseCompare(a, b, equals, this._secProp);
            } else {
                return this._compare(a, b, equals);
            }
        }
    }
    _reverseCompare(a, b, equals, property = this._prop){
        if (!property) {
            if (!equals) {
                if (this._order == "max") {
                    return (a < b);
    ...
    _bubbleUp(startIndex) {
        ...
        if (this._compare(this._arr[startIndex], this._arr[parentIndex], false, this._prop, true)) {
    ...
    isValidHeap(startIndex = 0) {
        ...
        let child1 = startIndex * 2 + 1;
        let child2 = child1 + 1;
        let arr = [this._arr[child1] === undefined || this._compare(this._arr[startIndex], this._arr[child1], true, this._prop, true), this.isValidHeap(child1)];
